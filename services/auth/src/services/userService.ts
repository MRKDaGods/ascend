...
export const findUserByEmail = async (email: string): Promise<User | null> => {
    const result = await pool.query('SELECT * FROM auth_service.users WHERE email = $1', [email]);
...
export const findUserById = async (id: number): Promise<User | null> => {
    const result = await pool.query('SELECT * FROM auth_service.users WHERE id = $1', [id]);
    return result.rows.length > 0 ? result.rows[0] : null;
};
...
export const createUser = async (
    ...
    isVerified: boolean = false): Promise<User> => {
    const password_hash = password ? await bcrypt.hash(password, 10) : '';
    const result = await pool.query(
        'INSERT INTO auth_service.users (first_name, last_name, email, password_hash, is_verified) VALUES ($1, $2, $3, $4, $5) RETURNING *',
        [firstName, lastName, email, password_hash, isVerified]
    );
 ...
 */
export const updateUserEmailConfirmation = async (email: string, token: string): Promise<User | null> => {
    const result = await pool.query(
 ...
 * @param token - The confirmation token to validate against the stored token
*/
export const updateUserNewEmailConfirmation = async (newEmail: string, token: string): Promise<User | null> => {
    const result = await pool.query(
 ...
 */
export const updateUserResetToken = async (email: string, resetToken: string): Promise<void> => {
    await pool.query('UPDATE auth_service.users SET reset_token = $1 WHERE email = $2', [resetToken, email]);
};
 ...
 */
export const resetUserPassword = async (email: string, token: string, newPassword: string): Promise<User | null> => {
...
export const updateUserPassword = async (id: number, newPassword: string): Promise<void> => {
    const password_hash = await bcrypt.hash(newPassword, 10);
    await pool.query('UPDATE auth_service.users SET password_hash = $1 WHERE id = $2', [password_hash, id]);
};
 ...
 */
export const updateUserEmail = async (id: number, newEmail: string, confirmationToken: string): Promise<void> => {
    await pool.query(
...
export const deleteUser = async (id: number): Promise<void> => {
    await pool.query('DELETE FROM auth_service.users WHERE id = $1', [id]);
};