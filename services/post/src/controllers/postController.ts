...
//           postService.addMediaToPost(post.id, {
//             file,
//             type: req.body.type,
...
export const getPostComments = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    const comments = await postService.getPostComments(postId, limit, (page - 1) * limit);
...
export const updateComment = [
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      const comment = await postService.updateComment(commentId, content, userId);
      if (!comment) {
        return res.status(404).json({ 
          success: false, 
...
// Utility Controllers
export const getPostEngagement = async (req: AuthenticatedRequest, res: Response) => {
  const postId = parseInt(req.params.postId);
...
export const getSavedPosts = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    const savedPosts = await postService.getSavedPosts(userId, limit, (page - 1) * limit);
...
export const searchPosts = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    const posts = await postService.searchPosts(query, limit, (page - 1) * limit);
