...
describe('PostService', () => {
  ...
  describe('toggleLike', () => {
    it('should like a post successfully', async () => {
      ...
      // Mock check for existing reaction (using reactions table)
      (db.query as jest.Mock).mockResolvedValueOnce({
        rows: [{ exists: false }]
      });
      ...
      const result = await postService.toggleLike(mockUserId, mockPostId);
      
      expect(result).toEqual({ liked: true });
      expect(db.query).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO post_service.reactions'),
        [mockUserId, mockPostId, 'like']
      );
    });
    ...
    it('should throw error when post not found', async () => {
      // Mock getPostById to return null
      (db.query as jest.Mock).mockResolvedValueOnce({
        rows: []
      });
      
      await expect(
        postService.toggleLike(123, 999)
  ...
  describe('reactToPost', () => {
    it('should add a reaction to a post successfully', async () => {
      ...
      (db.query as jest.Mock).mockResolvedValueOnce({}); // Update post_engagement
  ...
  describe('updatePrivacy', () => {
    it('should update post privacy successfully', async () => {
      const mockPostId = 1;
      ...
      // Mock likes, comments, shares counts
      (db.query as jest.Mock).mockResolvedValueOnce({ rows: [{ count: '0' }] });
      (db.query as jest.Mock).mockResolvedValueOnce({ rows: [{ count: '0' }] });
      (db.query as jest.Mock).mockResolvedValueOnce({ rows: [{ count: '0' }] });
      
      const result = await postService.updatePrivacy(mockPostId, mockPrivacy);
