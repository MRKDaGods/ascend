      ...
      body: BlocConsumer<PostBloc, PostState>(
        listener: (context, state) {
          ...
          if (state is PostsLoaded) {
            ...
            debugPrint(
              '✅ [SavedPostsPage] Loaded state received: ${state.posts.length} total posts, HasMore: ${state.hasMorePages}',
            );
        ...
        builder: (context, state) {
          ...
          if (state is PostsInitial ||
              (state is PostsLoading && !_isLoadingMore)) {
          ...
          if (state is PostsLoaded) {
            ...
            final savedPosts =
                state.posts.where((post) => post.isSaved).toList();
            final hasMorePages =
                state.hasMorePages; // Get pagination status from PostBloc state

            debugPrint(
              '📊 [SavedPostsPage] Builder using Loaded state: ${savedPosts.length} saved posts found, HasMore: $hasMorePages',
            );

            if (savedPosts.isEmpty && !_isLoadingMore) {
              debugPrint('ℹ️ [SavedPostsPage] Showing Empty state UI');
              return RefreshIndicator(
                // Allow refresh even when empty
                onRefresh: () async {
                  debugPrint(
                    '🔄 [SavedPostsPage] Refresh triggered. Adding LoadPosts.',
                  );
                ...
                child: LayoutBuilder(
                  // Ensure RefreshIndicator works with SingleChildScrollView
            ...
            return RefreshIndicator(
              ...
              child: CustomScrollView(
                // Use CustomScrollView like in home.dart
                ...
                slivers: [
                  ...
                  SliverList(
                    delegate: SliverChildBuilderDelegate(
                      ...
                      childCount:
                          savedPosts.length +
                          (_isLoadingMore || hasMorePages ? 1 : 0),
                    ),
                  ),
                ],
              ),
