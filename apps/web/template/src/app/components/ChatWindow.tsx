...
export default function ChatWindow() {
  ...
  const loadOlderMessages = () => {
    ...
    extApi.get(`messaging/conversations/${selectedConversationId}?limit=20&page=${nextPage}`)
      .then((res) => {
        ...
        setMessagesForConversation(selectedConversationId!, (prev) => [...newMessages.reverse(), ...prev]);
        setPage(nextPage);
      })
      ...
      <Box sx={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0, paddingTop: "62px", width: "100%" }}>
        ...
        >
          <Box sx={{ display: "flex", flexDirection: "column" }}>
            <Typography variant="subtitle1" fontWeight="bold" sx={{ mb: "2px", lineHeight: 1.1 }}>
              {partnerName}
            </Typography>
            {typingStatus && (
              <Typography variant="caption" color={theme.palette.text.secondary} sx={{ mt: "2px", lineHeight: 1 }}>
                typing...
              </Typography>
            )}
          </Box>
        ...
        {!isBlocked ? (
          ...
          <Box sx={{ padding: 2, textAlign: "center", color: theme.palette.text.disabled }}>
