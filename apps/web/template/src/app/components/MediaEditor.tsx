    ...
    <>
      ...
      <Dialog open={editorOpen} onClose={handleClose} fullWidth maxWidth="lg">
        <DialogTitle sx={{ display: "flex", justifyContent: "space-between" }}>
          <Typography fontWeight="bold" fontSize="1.5rem">Editor</Typography>
          <IconButton onClick={handleClose}><Close /></IconButton>
        </DialogTitle>
        <DialogContent sx={{ minHeight: "300px" }}>
          {mediaFiles.length > 0 ? <MediaPreview /> : (
            <Box textAlign="center" mt={5}>
              <img src="/select-files.png" alt="Illustration" width="250px" />
        ...
        {mediaFiles.length > 0 && (
          <Box sx={{ display: "flex", justifyContent: "flex-end", p: 2 }}>
            <Button variant="contained" onClick={closeEditor}>Next</Button>
          </Box>
        )}
      </Dialog>
      <DiscardDialog open={discardDialogOpen} onClose={closeDiscardDialog} onDiscard={handleDiscard} />
    </>
  );
};
