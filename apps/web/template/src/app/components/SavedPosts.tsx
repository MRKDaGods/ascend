...
const SavedPosts: React.FC = () => {
  ...
  return (
    <Box sx={{ maxWidth: 700, mx: "auto", px: 2, color: theme.palette.text.primary }}>
      ...
      {saved.map((post) => {
        ...
        const media = post.media?.find((m) => m.type === "image" || m.type === "video");

        return (
          <Box key={post.id} onClick={() => router.push(`/feed/${post.id}`)} sx={{ cursor: "pointer" }}>
            <Card sx={{ mb: 2, p: 2, borderRadius: 3, backgroundColor: theme.palette.background.paper, boxShadow: 2 }}>
